...
async def sing_up(mess):
    await mess.answer(text="Введите имя пользователя (только латинский алфавит):?")
...
@disp.message_handler(text=RegistrationState.username)
async def set_username(mess, state):
    await mess.answer(text="Введите имя пользователя (только латинский алфавит):")
    if not is_included(mess):
        add_users(mess.text)
    await mess.answer(text="Введите свой email:")
